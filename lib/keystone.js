...
Keystone.prototype.listUsers = function(access_token, cb)
{
  var self = this;
  var request_options = {};
  
  request_options = this.getRequestOptions(access_token, '/users', true);
  request_options.metricPath = 'remote-calls.keystone.users.list';
  request_options.validateStatus = true;
  request_options.requireBodyObject = 'users';
  ...
  this.request.get(request_options, function(error, response, body){
    ...
    for(var n = 0; n < body.users.length; n++)
    {
      users_array[n] = self.mangleObject('User', body.users[n]);
    }

    //tack these on for easy consupmtion and in case we ever need pagination
    users_array.self = body.links.self;
    users_array.previous = body.links.previous;
    users_array.next = body.links.next;
...
Keystone.prototype.addUser = function(access_token, name, extra, cb)
{
  var self = this;
  var request_options = {};
  var user_object = {};
  ...
  // optional arguments
  if (extra)
    Object.assign(user_data.user, extra)

  request_options = this.getRequestOptions(access_token, '/users', user_data);
  request_options.metricPath = 'remote-calls.keystone.users.add';
  request_options.validateStatus = true;
  request_options.requireBodyObject = 'user';
  ...
  this.request.post(request_options, function(error, response, body){
    ...
    user_object = self.mangleObject('User', body.user);
...
Keystone.prototype.updateUser = function(access_token, user_id, extra, cb)
{
  var self = this;
  var request_options = {};
  var user_object = {};
  ...
  request_options = this.getRequestOptions(access_token, '/users/' + user_id, user_data);
  request_options.metricPath = 'remote-calls.keystone.users.update';
  request_options.validateStatus = true;
  request_options.requireBodyObject = 'user';
...
Keystone.prototype.removeUser = function(access_token, user_id, cb)
{
  var request_options = this.getRequestOptions(access_token, '/users/' + user_id, true);
  request_options.metricPath = 'remote-calls.keystone.users.remove';
  request_options.validateStatus = true;
...
Keystone.prototype.getUser = function(access_token, user_id, cb)
{
  var self = this;
  var user_object = {};

  var request_options = this.getRequestOptions(access_token, '/users/' + user_id, true);
  request_options.metricPath = 'remote-calls.keystone.users.get';
  request_options.validateStatus = true;
  request_options.requireBodyObject = 'user';
